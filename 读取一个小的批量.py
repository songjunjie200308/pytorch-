...
true_b = 4.2
features, labels = synthetic_data(true_w, true_b, 1000)#代入w和b生成1000个样本
...
def data_iter(batch_size, features, labels):
    ...
    indices = list(range(num_examples))  # 样本索引即生成0到1000的整数数列
    # 这些样本是随机读取的，没有特定的顺序
    random.shuffle(indices)  # 把索引随机打乱
    for i in range(0, num_examples, batch_size):
        batch_indices = torch.tensor(
            indices[i:min(i + batch_size, num_examples)])  # 当i+batch_size超出时，取num_examples
        yield features[batch_indices], labels[batch_indices]  # 这里传入一个10个整数的数组，系统会自动根据传入数组中的每个数作为下标取出对应的10个元素
